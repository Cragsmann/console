...
test.describe('Clone Tests', { tag: ['@admin', '@storage'] }, () => {
  ...
  test('creates and deletes a PVC clone', async ({ page, k8sClient, cleanup }) => {
    ...
    await test.step('Clone PVC', async () => {
      await listPage.clickRowKebabAction(pvcName, 'Clone PVC');
    ...
    await test.step('Verify clone appears in list', async () => {
      ...
      await listPage.rowShouldExist(cloneName);
    });

    await test.step('Delete clone', async () => {
      await listPage.filterByName(cloneName);
      await listPage.clickRowKebabAction(cloneName, 'Delete PersistentVolumeClaim');
      await modal.shouldBeOpened();
      await modal.submitShouldBeEnabled();
      await modal.submit();
      await modal.shouldBeClosed();
      await listPage.rowShouldNotExist(cloneName);
    });
  });
